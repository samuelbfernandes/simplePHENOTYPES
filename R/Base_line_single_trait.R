...
Base_line_single_trait <-
  function(additive.object = NULL,
    ...
    additive.component <-
      as.data.frame(matrix(0, nrow = nrow(additive.object), ncol = 1))
    
    additive.component <-
      additive.component + (additive.object[, 1] * (big.additive.QTN.effect))
    
    addnumber <- ncol(additive.object)
    if (addnumber >= 2){
      for (i in 2:addnumber) {
        additive.component <-
          additive.component + (additive.object[, i] * (additive.effect[1] ^ (i - 1)))
      }
    }  #'end if(addnumber >= 2)
    ...
    if (!is.null(epistatic.object)) {
      ...
      epistatic.component <-
        as.data.frame(matrix(0, nrow = nrow(epistatic.object), ncol = 1))
      
      enumber <- ncol(epistatic.component)
      for (i in 0:(enumber - 1)) {
        epistatic.component <-
          epistatic.component + ((epistatic.object[, ((2 * i) + 1)] * 
