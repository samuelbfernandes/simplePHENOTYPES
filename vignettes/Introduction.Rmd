...
Note that the example dataset is alredy numericalized. HapMap format may also be used with options `file.G` and `file.Ext.G`.
```{r load data}
...
# Using your own data
If files are saved by chromosome, they may be readed from file using options `file.G`, `file.Ext.G`, `file.from` and `file.to`. For example, if your data is saved as "WGS_chrm_1.hmp.txt", ..., "WGS_chrm_10.hmp.txt", one would load it with the following code:
